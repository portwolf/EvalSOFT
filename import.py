...
l = 10.6/1000					    ### Breite eines Streifens im Streifenmode
...
df = dfRead.as_matrix()                             ### Konvertiere eingelesene
###print (df)                                       ### Daten in Matrix

### Initialisiere Gewichtungsfunktion Matrix
GFkt = np.zeros((1000, 1000))

### Schleife um GFkt mit den Elementabstaenden zu fuellen
for i in range(len(df)):
    
    for j in range(len(df)):
        GFkt[i, j] = abs((df[i, 0] - df[j, 0])*l)
        print(GFkt[i,j])
    print(i)
